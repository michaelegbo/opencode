...
const TEMPLATE_REFERENCE_EXCLUDED = /(^|\/)(node_modules|dist|build|coverage|\.next|\.turbo|\.vite)(\/|$)|(^|\/)(package-lock\.json|npm-shrinkwrap\.json|pnpm-lock\.yaml|yarn\.lock|bun\.lock|bun\.lockb)$|\.(png|jpe?g|gif|webp|avif|ico|svg|woff2?|ttf|eot|mp4|webm|mov|zip|gz|br|pdf)$/i
...
const formatTemplateNote = (item: TemplateContextItem) => {
  ...
  lines.push("Apply the selected template faithfully: preserve its visible layout, spacing, hierarchy, colors, and interaction intent while fitting the current codebase.")
